...
control Crosspod(inout headers_t hdr, inout booster_metadata_t m, inout metadata_t meta) {
    ...
    apply {
      bool fp_is_active = false;
      if (hdr.ipv4.isValid()) {
        fp_get_Active(hdr, fp_is_active);
      }

      if (!fp_is_active) {
        drop(); // We're not expecting other traffic
      } else {
//        #include "FPCheckFeedbackHP.p4"
        fp_get_ToSegment(hdr, fp_to_segment);
        fp_original_to_segment = fp_to_segment;
        if (2 == fp_to_segment) {
            init_computation(hdr);
            set_computation_order(hdr, computation_continuing, 3, fp_to_segment, meta);
        } else if (4 == fp_to_segment) {
            init_computation(hdr);
            set_computation_order(hdr, computation_continuing, 5, fp_to_segment, meta);
        } else {
          // FIXME fail
        }
        offload_port_lookup.apply();

        if (2 == fp_original_to_segment) {
...
#endif
        } else if (4 == fp_original_to_segment) {
#if defined(COMPRESSION_BOOSTER)
            bit<1> compressed_link = 0;
            bit<1> forward = 0;
