...
if [ -z "${TOPOLOGY}" ]
then
  export TOPOLOGY=$WHARF_REPO/splits3/ALV_bt/alv_k=4.yml
...
MODES=(bt_experiment_base bt_experiment_encapsulated)
DEFAULT_MODE=bt_experiment_base
...
function bt_experiment_encapsulated {
  if [ -z "${NUM_PINGS}" ]
  then
    NUM_PINGS=30
  fi
  ...
  sudo -E python $WHARF_REPO/bmv2/start_flightplan_mininet.py ${TOPOLOGY} \
          ...
          --showExitStatus \
     --fg-host-prog ": ${EXPERIMENT_INIT}" \
     --fg-host-prog "p0h3: $WHARF_REPO/splits3/ALV_bt/send.sh ${NUM_PINGS}" \
          2> $LOG_DUMPS/flightplan_mininet_log.err
     # where:
     #   p0h3 = 192.0.1.3
     #   p3h2 = 192.3.1.2
