...
# start dpdk.
DPDK_SCRIPT_DIR=/home/"$real_user"/gits/dpdkScripts
cd $DPDK_SCRIPT_DIR
. ./setDpdkPaths.bash
./allocHugePages.sh
cd $PKTGEN_DIR
...
mv $PKTGEN_DIR/throughput.txt "$EXP_DIR"/"$BENCH_DIR"/pcapThroughput."$EXP_NAME".txt

cd $EXP_DIR

# 2. benchmark booster with no encoder.
EXP_NAME=noFec
BOOSTER_NAME=fecEncodeBooster_baseline
echo "running experiment: $EXP_NAME"
...
# 3. benchmark booster with encoder.
EXP_NAME=fec
