...
void tofinoHandler(u_char *userData, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
    ...
    uint outPktLen = sizeof(ether_header) + sizeof(fec_header) + plLen;
    u_char outPkt[outPktLen];


    // Fill fec header.
    fecHeader.pktId = pktId;
    fecHeader.blockId = blockId;

    // Build tagged data packet to send out.
    ethHeader = (struct ether_header *) packet;  
    ethPayload = (u_char *) packet + sizeof(ether_header);
    memcpy(outPkt, ethHeader, sizeof(ether_header));
    memcpy(outPkt+sizeof(ether_header), &fecHeader, sizeof(fec_header));
    memcpy(outPkt+sizeof(ether_header)+sizeof(fec_header), ethPayload, plLen);
    ...
    pcap_inject(pcapInterface, outPkt, sizeof(*ethHeader)+sizeof(fecHeader)+plLen);
    pktId++;
