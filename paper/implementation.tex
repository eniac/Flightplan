...
\begin{figure}
  ...
  \hg{I find this diagram confusing.  "SDNet" is not a language as opposed to
      P4 and C.  In reality, the packets go from P4
      to a user engine that is written in Verilog.  The user engine
      is a simple wrapper that in turn forwards the packets to C.}
  \lei{SDNet IS a language. We can absolutely implement everything P4 does solely in SDNet.}
  \hg{The language is called PX, not SDNet.  P4 is translated into PX, so packets never
      go from P4 to PX.  If packets are in P4, they are in PX, and vice versa.}
\end{figure}
