...
state, processing packet stream, etc -- to which we can add our external function. 
%This allowed us to implement our function as a stream processor, for cut-through behaviour through SDNet.
As is shown by Fig.~\ref{fig:sdnet-interface}, we are able to implement
...
\begin{figure}
  ...
  \caption{\label{fig:sdnet-interface}Abstract provided by each layer; grey blocks are translated to the pipeline illustrated in next level.}
\end{figure}
