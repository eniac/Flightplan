...
\hg{As the figure with the topology no longer exists, I would remove the top
    part of this diagram because S1 and S2 have not meaning anymore.  Besides,
    it was only saying that only faulty links use tagged packets, which seems
    obvious.  What is mentioned as "original header" should just
    be the original "EtherType" field only by the way.  I would also indicate in
    the figure what the \OurSys tag is.}
\end{figure}
...
\begin{figure}
  ...
  \hg{I suppose we also classify according to flow (source-destination pair)?
      S1 is not referring to anything anymore as we removed the first figure.}
\end{figure}
