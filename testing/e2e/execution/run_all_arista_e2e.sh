...
OUTPUT=../e2e_output_500k_arista_2
...
run_shremote() {
    ...
    for i in $(seq 1 5); do
        ../../Shremote/shremote.py cfgs/arista_e2e_500k.yml ${BASE_LABEL}_$ITER \
            --args "drop_rate:$DROP_RATE;dataplane_flags:$FLAGS" \
            --out $OUTPUT/$BASE_LABEL/ --delete;
        rtn=$?
        if [[ $rtn != 0 ]]; then
            echo "Execution failed... trying again";
            curl -X POST -H 'Content-type: application/json' --data "{\"text\":\"Experiment $BASE_LABEL $ITER failed :( \"}"
            sleep 2
        else
            break
        fi
...
curl -X POST -H 'Content-type: application/json' --data "{\"text\":\"Experiment set $SUFFIX succeeded!! \"}"
