...
static uint64_t currentFECBlock = 0; 
static int totalPacketsRecovered = 0;
...
void PacketSink::StopApplication ()     // Called at time specified by Stop
{
  ...
  if (m_socket) 
    ...
    double ratePacketsRecovered = (double)totalPacketsRecovered/(double)totalPacketsLost;
    cout << "Packet loss avoided : " << ratePacketsRecovered << endl;
}
...
void PacketSink::HandleRead (Ptr<Socket> socket)
{
  ...
  ParityTag pTag;
  DataTag dTag;
  uint64_t parityBlock, dataBlock;
  ...
  while (packet = socket->RecvFrom (from))
	...
	if (packet->RemovePacketTag(pTag)) {
		parityBlock = pTag.GetPid();
		parityFlag = true;
		parityCounter++;
	} else if (packet->RemovePacketTag(dTag)) {
		dataBlock = dTag.GetPid();
 		if ((dataBlock != currentFECBlock) && currentFECBlock != 0) {
		//Examine no. of data+parity packets received
	
			int dataPacketsLost = k - dataCounter;
			totalPacketsLost += dataPacketsLost;
			if (dataPacketsLost == 0) {
				//cout << "No packets lost - " << currentFECBlock << endl;

			} else if (parityCounter >= dataPacketsLost) {
				//cout << "Lost packet recovered - " << currentFECBlock << endl;
				totalPacketsRecovered += dataPacketsLost;
			} else {
				//cout << "Packets lost - " << currentFECBlock<<endl;
			}
			currentFECBlock = dataBlock;
			dataCounter = 0;
			parityCounter = 0;
		} else if (currentFECBlock == 0) {
			currentFECBlock = dataBlock;
		}
		dataCounter++;
	 }
