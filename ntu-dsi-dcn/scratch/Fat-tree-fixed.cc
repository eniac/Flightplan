...
//
	int k = 8;			// number of ports per switch
...
// Initialize parameters for On/Off application
	...
	int port = 9;
	int packetSize = 64;		// 1024 bytes
	char dataRate_OnOff [] = "8000Mbps";
	char maxBytes [] = "0";		// unlimited

// Initialize parameters for Csma and PointToPoint protocol

	char dataRate [] = "10000Mbps";	// 1Gbps
	int delay = 0.0001;		// 0.001 ms
	double errorRate = 0.1;

// Set parameters and Initialize objects for error model - packet erasure on Ethernet
       ...
       string errorModelType = "ns3::RateErrorModel";
...
//	
	...
	for (i=0;i<num_pod;i++){
		for (j=0;j<num_edge; j++){
			...
			//	hostSw[i][j][h] = 
					p2p.Install (NodeContainer (host[i][j].Get(h), 
					    edge[i].Get(j)));
				//if (h == 0) {
				//	hostSw[i][j].Add(link.Get(1));
				//}
				//hostSw[i][j].Add(link.Get(0));
				hostSw[i][j].Add(link);
			}
...
//	
	...
	for (i=0;i<num_pod;i++){
		for (j=0;j<num_edge; j++){
			for (h=0; h< num_host;h++){			
				NetDeviceContainer link = p2p.Install(edge[i].Get(j), host[i][j].Get(h));
				//hostSw[i][j] = p2p.Install(edge[i].Get(j), host[i][j].Get(h)); 
				hostSw[i][j].Add(link.Get(1));
			}
	...
	for (i=0;i<num_pod;i++){
		for (j=0;j<num_agg;j++){
			for (h=0;h<num_edge;h++){
				...
				ae[i][j][h].Get(0)->SetAttribute("ReceiveErrorModel", PointerValue(em));
	...
	for (i=0; i<num_group; i++){		
		for (j=0; j < num_core; j++){
			...
			for (h=0; h < num_pod; h++){			
				...
				ca[i][j][h].Get(0)->SetAttribute("ReceiveErrorModel", PointerValue(em));
...
//
	...
	for (i=0;i<1;i++){
		app[i].Start (Seconds (0.0));
  		app[i].Stop (Seconds (3.0));
	}
 	Ipv4GlobalRoutingHelper::PopulateRoutingTables ();

// Set up sink app at hosts to consume incoming traffic
//
    	...
    	for (int i = 0; i < num_pod; ++i) {
        	for (int j = 0; j < num_edge; ++j) {
            		for (int k = 0; k < num_host; ++k) {
                		udpSinkHost.Add(host[i][j].Get(k));
            		}
        	}
    	}
    	ApplicationContainer udpSinkApps = udpSinkHelper.Install(udpSinkHost);
    	udpSinkApps.Start(Seconds(0.0));
   	udpSinkApps.Stop(Seconds(3.0));
